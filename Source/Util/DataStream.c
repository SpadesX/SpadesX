...
{
    return (stream->offset < stream->length) ? stream->length - stream->offset : 0;
}

void stream_skip(stream_t* stream, uint32_t skip)
{
    stream->offset = (stream->offset + skip < stream->length) ? stream->offset + skip : stream->length;
}
...
uint16_t stream_read_u16(stream_t* stream)
{
    ...
    uint16_t value = 0;
    value |= ((uint16_t) stream->data[stream->offset++]);
    value |= ((uint16_t) stream->data[stream->offset++]) << 8;
    return value;
}
...
uint32_t stream_read_u32(stream_t* stream)
{
    ...
    uint32_t value = 0;
    value |= ((uint32_t) stream->data[stream->offset++]);
    value |= ((uint32_t) stream->data[stream->offset++]) << 8;
    value |= ((uint32_t) stream->data[stream->offset++]) << 16;
    value |= ((uint32_t) stream->data[stream->offset++]) << 24;
    return value;
}
...
{
    color_t color;
    if (stream->offset + 3 > stream->length) {
        color.raw = 0;
        return color;
    }
    color.a = 0xFF;
    color.r = stream->data[stream->offset++];
    color.g = stream->data[stream->offset++];
    color.b = stream->data[stream->offset++];
    return color;
}
...
color_t stream_read_color_argb(stream_t* stream)
{
    ...
    color.a = stream->data[stream->offset++];
    color.r = stream->data[stream->offset++];
    color.g = stream->data[stream->offset++];
    color.b = stream->data[stream->offset++];
    return color;
...
void stream_read_array(stream_t* stream, void* output, uint32_t length)
{
    ...
    memcpy(output, stream->data + stream->offset, length); // Replace me
...
{
    ACCESS_CHECK_N(stream, length);
    memcpy(stream->data + stream->offset, array, length); // Replace me
    stream->offset += length;
}
...
void stream_write_u16(stream_t* stream, uint16_t value)
{
    ...
    stream->data[stream->offset++] = (uint8_t) value;
    stream->data[stream->offset++] = (uint8_t) (value >> 8);
}
...
void stream_write_u32(stream_t* stream, uint32_t value)
{
    ...
    stream->data[stream->offset++] = (uint8_t) value;
    stream->data[stream->offset++] = (uint8_t) (value >> 8);
    stream->data[stream->offset++] = (uint8_t) (value >> 16);
    stream->data[stream->offset++] = (uint8_t) (value >> 24);
}
...
void stream_write_color_rgb(stream_t* stream, color_t color)
{
    ...
    stream->data[stream->offset++] = color.b;
    stream->data[stream->offset++] = color.g;
    stream->data[stream->offset++] = color.r;
}
...
void stream_write_color_argb(stream_t* stream, color_t color)
{
    ...
    stream->data[stream->offset++] = color.g;
    stream->data[stream->offset++] = color.r;
    stream->data[stream->offset++] = color.a;
}

void stream_write_color_3u8(stream_t* stream, uint8_t r, uint8_t g, uint8_t b)
{
    ACCESS_CHECK_N(stream, 3);
    stream->data[stream->offset++] = b;
    stream->data[stream->offset++] = g;
