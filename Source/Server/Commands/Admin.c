...
void cmd_admin(void* p_server, command_args_t arguments)
{
    ...
    if (arguments.argc > 1) {
        if (arguments.console == 0 && arguments.player->admin_muted == 1) {
            send_server_notice(arguments.player, arguments.console, "You are not allowed to use this command (Admin muted)");
            return;
        }
        if (arguments.console) {
            ...
            server, "Staff from %s: %s", arguments.player->name, arguments.argv[1]);
        }
