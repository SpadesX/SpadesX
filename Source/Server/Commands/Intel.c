...
void cmd_intel(void* p_server, command_args_t arguments)
{
    ...
    if (sentAtLeastOnce == 0) {
        if (server->protocol.gamemode.intel_held[0]) {
            send_server_notice(arguments.player, arguments.console, "Intel is not being held but intel of team 0 thinks it is");
        } else if (server->protocol.gamemode.intel_held[1]) {
            send_server_notice(arguments.player, arguments.console, "Intel is not being held but intel of team 1 thinks it is");
        }
