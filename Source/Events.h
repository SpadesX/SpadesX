...
//
// Defined functions:
// - void [event]Subscribe([event]Callback callback, int position) - adds a callback to the list. Position is either EVENT_FIRST or EVENT_LAST
// - void [event]Unsubscribe([event]Callback callback) - find given callback in the list and remove it if it's present
// - void [event]Run(...) - called when the event had occured. Calls every callback until either callback returns EVENT_BREAK or there will be no more callbacks in the list
#define EVENT(event, ...) \
...
// Arguments:
// - event - same as EVENT
// - argumentNames - same as ... in EVENT but in brackets and without the type names, pointer signs etc (example: (Name, Number, Data))
// - ... - same as EVENT
